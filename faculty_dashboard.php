...
// Get stats
$sessions_count = mysqli_query($conn, "SELECT COUNT(*) as count FROM sessions WHERE faculty_id = '$faculty_id'");
$total_sessions = mysqli_fetch_assoc($sessions_count)['count'] ?? 0;

$attendance_count = mysqli_query($conn, 
    "SELECT COUNT(*) as total FROM attendance_records ar 
     JOIN sessions s ON ar.session_id = s.session_id 
     WHERE s.faculty_id = '$faculty_id'");
$total_attendance = mysqli_fetch_assoc($attendance_count)['total'] ?? 0;

$active_sessions = mysqli_query($conn, "SELECT COUNT(*) as count FROM sessions WHERE faculty_id = '$faculty_id' AND is_active = 1");
$active_count = mysqli_fetch_assoc($active_sessions)['count'] ?? 0;
