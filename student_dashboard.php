...
if ($conn) {
    ...
    if ($student) {
        ...
        // FIRST: Get total attendance count (filtered by student's section)
        if ($student_section) {
            ...
            if ($stmt_total) {
                mysqli_stmt_bind_param($stmt_total, "ss", $student_id, $student_section);
        ...
        if ($student_section) {
            // Get subjects that have sessions in the student's section
            $subjects_query = "SELECT DISTINCT s.subject_name, s.subject_code, s.target_sessions 
                FROM subjects s 
        ...
        // Fallback if no subjects found for the section
        if (empty($subjects_data)) {
            ...
            // Also fetch all subjects without section filter as fallback
            $fallback_query = "SELECT subject_name, subject_code, target_sessions FROM subjects";
            $fallback_result = mysqli_query($conn, $fallback_query);
            if ($fallback_result && mysqli_num_rows($fallback_result) > 0) {
                while ($subject = mysqli_fetch_assoc($fallback_result)) {
                    $subjects_data[] = [
                        'name' => $subject['subject_name'],
                        'code' => $subject['subject_code'],
...
// ==================== GET SESSIONS HAPPENED - FILTERED BY SECTION ====================
$sessions_happened = 0;
if ($conn && isset($student_section)) {
    // Count how many sessions have actually happened for the student's section (past sessions)
    ...
    if ($stmt_happened) {
        ...
        if ($happened_row = mysqli_fetch_assoc($happened_result)) {
            $sessions_happened = intval($happened_row['total_happened']);
        }
    ...
    <div class="row mb-4">
        <div class="col-12">
            <h2 class="mb-2">
                ...
                Hi, <?php echo htmlspecialchars($student['student_name'] ?? 'Student'); ?>!
            </h2>
            <p class="text-muted">Welcome to your smart attendance dashboard</p>
        </div>
    </div>
            ...
            <div class="card shadow-lg border-0">
                ...
                <div class="card-body text-center">
                    ...
                    <div class="mb-3">
                        ...
                        ?>
                        <img src="<?php echo htmlspecialchars($profile_pic); ?>" class="rounded-circle profile-img border" alt="Profile Picture" onerror="this.onerror=null; this.src='uploads/profiles/default.png';">
                    </div>
                    ...
                    <p class="card-text text-muted">
                        ...
                        <i class="fas fa-<?php echo ($gender === 'female') ? 'venus' : 'mars'; ?> me-1"></i> <?php echo ucfirst(htmlspecialchars($gender)); ?>
                    </p>
                </div>
            </div>
                ...
                <div class="card-body text-center">
                    <?php if (!empty($student['qr_content'])): ?>
                        ...
                        <p class="mt-2 small text-muted">
                            <i class="fas fa-info-circle me-1"></i> Show this QR code during class to mark attendance
                        </p>
                    ...
                    <?php else: ?>
                        <div class="alert alert-warning">
                            <i class="fas fa-exclamation-triangle me-2"></i> QR Code not generated yet. Contact administrator.
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="row mb-4">
                <div class="col-md-4 mb-3">
                    <div class="stat-card stat-card-1">
                        ...
                        <small>Out of <?php echo $total_possible_sessions; ?> total (Section <?php echo htmlspecialchars($student['section'] ?? 'N/A'); ?>)</small>
                    </div>
                </div>
                <div class="col-md-4 mb-3">
                    <div class="stat-card stat-card-2">
                        ...
                        <div class="small mb-2">
                            <?php if ($sessions_happened > 0): ?>
                                (<?php echo $total_attendance; ?>/<?php echo $sessions_happened; ?> sessions in Section <?php echo htmlspecialchars($student['section'] ?? 'N/A'); ?>)
                            <?php else: ?>
                                (No sessions yet in your section)
                            <?php endif; ?>
                        </div>
            ...
            <div class="predictor-card">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        ...
                        <p class="mb-0">Track your progress towards 75% attendance requirement for Section <?php echo htmlspecialchars($student['section'] ?? 'N/A'); ?></p>
                    </div>
                ...
                <div class="calculation-box mt-3">
                    ...
                    <div class="mt-3">
                        ...
                        <div class="progress" style="height: 10px;">
                            <div class="progress-bar bg-warning" role="progressbar" style="width: <?php echo $sessions_for_75_percent > 0 ? min(100, ($total_attendance / $sessions_for_75_percent) * 100) : 0; ?>%" aria-valuenow="<?php echo $sessions_for_75_percent > 0 ? min(100, ($total_attendance / $sessions_for_75_percent) * 100) : 0; ?>" aria-valuemin="0" aria-valuemax="100">
                            </div>
                        </div>
                    </div>
                    ...
                    <?php if ($remaining_for_75_percent > 0): ?>
                        <div class="alert alert-light mt-3 mb-0">
                            ...
                            <strong>Goal:</strong> Attend <?php echo $remaining_for_75_percent; ?> more sessions in Section <?php echo htmlspecialchars($student['section'] ?? 'N/A'); ?> (<?php echo ceil($remaining_for_75_percent / 8); ?> weeks at 100% attendance)
                        </div>
                    <?php else: ?>
                        <div class="alert alert-success mt-3 mb-0">
                            <i class="fas fa-trophy me-2"></i>
                            <strong>Congratulations!</strong> You've already achieved 75% attendance requirement for your section!
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="card shadow-lg border-0 mb-4">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <?php if ($sessions_happened > 0): ?>
                                <div class="progress attendance-progress">
                                    <div class="progress-bar bg-<?php echo $attendance_class; ?>" role="progressbar" style="width: <?php echo min($attendance_percentage, 100); ?>%" aria-valuenow="<?php echo $attendance_percentage; ?>" aria-valuemin="0" aria-valuemax="100">
                                    </div>
                                </div>
                                <div class="progress-percentage text-center">
                                    <?php echo $attendance_percentage; ?>% (<?php echo $total_attendance; ?>/<?php echo $sessions_happened; ?> sessions in Section <?php echo htmlspecialchars($student['section'] ?? 'N/A'); ?>)
                                </div>
                            <?php else: ?>
                                <div class="alert alert-warning">
                                    <i class="fas fa-info-circle me-2"></i>
                                    No class sessions have been scheduled yet for Section <?php echo htmlspecialchars($student['section'] ?? 'N/A'); ?>.
                                </div>
                            <?php endif; ?>

                            <!-- Subjects Breakdown -->
                            <div class="subjects-breakdown mt-3">
                                <h6><i class="fas fa-book me-2"></i>Subject Breakdown for Section <?php echo htmlspecialchars($student['section'] ?? 'N/A'); ?>:</h6>
                        ...
                        <div class="col-md-6">
                            <h6><i class="fas fa-chart-pie me-2"></i>Attendance Distribution for Section <?php echo htmlspecialchars($student['section'] ?? 'N/A'); ?></h6>
                            ...
                            <div class="alert alert-info mt-3">
                                ...
                                <strong>Section Filter Applied:</strong> Your attendance is calculated only for sessions in Section <?php echo htmlspecialchars($student['section'] ?? 'N/A'); ?>.
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="timetable-card">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            ...
                            <p class="mb-0">View your weekly class schedule and upcoming sessions for Section <?php echo htmlspecialchars($student['section'] ?? 'N/A'); ?>.</p>
                        </div>
                        <div class="col-md-4 text-center">
            ...
            <div class="card shadow-lg border-0">
                <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-light text-dark">
                        <i class="fas fa-user-check me-1"></i> <?php echo $total_attendance; ?> Records
                    </span>
                </div>
                <div class="card-body">
                        ...
                        <div class="recent-attendance-scroll">
                            <div class="table-responsive">
                                <table class="table table-hover attendance-table">
                                    ...
                                    <tbody>
                                        <?php mysqli_data_seek($attendance_result, 0); ?>
